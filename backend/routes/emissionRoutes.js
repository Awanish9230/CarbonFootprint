...
router.get('/summary', auth, async (req, res) => {
  ...
  const aggregateBreakdown = totals.reduce(
    ...
    { vehicleKm: 0, electricityKwh: 0, shoppingSpend: 0, foodKgCO2e: 0, other: 0 }
  );
...
router.get('/search-item', auth, async (req, res) => {
  ...
  // Alternatives: pick 5-8 others from same or related categories
  const categories = [...new Set(items.map((i) => i.category))];
  const alternatives = await EmissionFactor.find({ category: { $in: categories } }).limit(12);
  res.json({ items, alternatives: alternatives.filter((a) => !items.some((i) => i.id === a.id)).slice(0, 8) });
});
...
router.get('/recommendations', auth, async (req, res) => {
  ...
  const summary = logs.reduce(
    ...
    { totalCO2: 0, breakdown: { vehicleKm: 0, electricityKwh: 0, shoppingSpend: 0, foodKgCO2e: 0, other: 0 } }
  );
  const recs = await getRecommendations(summary);
  res.json({ range, recommendations: recs });
});

...
export default function LoginSignup() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      else await register({ name: form.name, email: form.email, password: form.password, state: form.state });
      navigate('/dashboard');
    } catch (err) {
  ...
  if (authLoading) return <div className="text-center mt-10 text-white">Loading...</div>;
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0a0a0a] p-4">
      <div className="w-full max-w-md bg-[#111111] rounded-2xl shadow-lg p-6 space-y-5">
        ...
        <p className="text-center text-gray-400 text-sm mt-2">
          {mode === 'login' ? "Don't have an account? Sign up above" : 'Already have an account? Login above'}
        </p>
      </div>
    </div>
  );
